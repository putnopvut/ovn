                       ...
                       const struct nbrec_acl *acl)
{
    ...
    if (!strcmp(acl->direction, "from-lport")) {
        if (smap_get_bool(&acl->options, "apply-after-lb", false)) {
            if (acl->tier > ls_stateful_rec->max_acl_tier.ingress_post_lb) {
                ls_stateful_rec->max_acl_tier.ingress_post_lb = acl->tier;
            }
        } else if (acl->tier > ls_stateful_rec->max_acl_tier.ingress_pre_lb) {
