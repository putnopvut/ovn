                       ...
                       const struct sbrec_datapath_binding *sb_dp)
{
    ...
    if (!datapath_get_nb_uuid_and_type(sb_dp, &nb_uuid, &dp_type)) {
        return NULL;
    }

    if (dp_type == DP_MAX || dp_type == DP_UNSPEC) {
                              ...
                              const struct ovn_unsynced_datapath *udp)
{
    ...
    sbrec_datapath_binding_set_type(sb_dp, udp->type);
}
...
create_synced_datapath_candidates_from_nb(
    ...
    struct vector *candidate_sdps)
{
    for (size_t i = DP_SWITCH; i < DP_MAX; i++) {
        ...
        if (sbrec_datapath_binding_is_new(sb_dp)) {
            if (!sdp) {
                ...
                if (sdp->pending_sb_dp) {
                    ...
                    /* Some jerk has inserted a duplicate datapath to try to
                     * fool us. Not on my watch!
                     */
                    return EN_UNHANDLED;
                }
            }
