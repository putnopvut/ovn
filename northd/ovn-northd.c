...
build_stateful(struct ovn_datapath *od, struct hmap *lflows, struct hmap *lbs)
{
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_IN_STATEFUL, 100,
                  REGBIT_CONNTRACK_COMMIT" == 1", "ct_commit { ct_label.blocked = 0; }; next;");
...
{
    const struct nbrec_logical_router_static_route *static_route = route->route;
    struct ds match = DS_EMPTY_INITIALIZER;
    struct ds actions = DS_EMPTY_INITIALIZER;
    ...
    ds_put_format(&actions, "ct_commit { ct_label.ecmp_reply_eth = eth.src;"
                  " ct_label.ecmp_reply_port = %" PRId64 ";}; next;", out_port->sb->tunnel_key);
    ...
    ovn_lflow_add_with_hint(lflows, od, S_ROUTER_IN_ARP_RESOLVE,
                            ...
                            ds_cstr(&actions), &static_route->header_);
}
